...
// Состояние ДАДМ:
// 0 - норма,
// 1 - проверка цепи при старте прошла успешно,
// 8 - нет давления масла,
// 16 - цепь ДАДМ неисправна.
...
// Переменная для обнуления дистанции.
float DistDelta = 0.0;

// Израсходованно топлива за поездку в мл
float FuelBurnedRide = 0.0;
// Переменная для обнуления топлива.
float FuelDelta = 0.0;
// Коэффициент коррекции расхода топлива /100
byte FuelConsumptionRatio = 111;
...
// Температура EGT, глобальная переменная для усреднения.
float EGTAVG = -101;
